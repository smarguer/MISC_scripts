...
open (OUT, ">", "$in.annot.$gff") or die 'could not open output file';
...
if ($type eq "genes")
{
 ...
 #if ($holder1[0] eq "gene_id"){next;}
 $strand='+';
 $chr=substr($holder1[2],2,1);
 $seg=$holder1[0];
 $i=0;
 $counthits=0;
 ...
 if ($type eq "jun")
{
 ...
 #if ($holder1[0] eq "gene_id"){next;}
 $strand=$holder1[5];
 $chr=substr($holder1[0],2,1);
 $seg=$holder1[3];
 $i=0;
 $counthits=0;
 @holder3= ($holder1[1],$holder1[2]);
 if ($holder3[0] > $holder3[1])
 {
  @holder3=reverse(@holder3);
 }
...
print OUT "$line1\t".join ("\t", keys %holder)."\n";
