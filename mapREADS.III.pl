...
open (OUT, ">", "$in.mapIII.$gff") or die 'could not open output file';
########################
...
#{
# print "$seg"."\n"; 
# print join("\t", keys %holder)."\n"; 
# $c=1;
#}
 
 $counthits= keys %holder;
 ...
 if ($FINALHIT{$seg} eq "empty")
 {
  $FINALHIT{$seg}=join ("\t", keys %holder);
  $out{$seg}="$seg\t$holder3[0]\t$holder3[1]\t$strand\t$chr";
 }
 else
 {
  $FINALHIT{$seg}.="\t".join ("\t", keys %holder);
  $out{$seg}="$seg\tMULTIPLE\tMULTIPLE\tMULTIPLE\tMULTIPLE";
 }
# if($c==1)
# {
# print "$FINALHIT{$seg}\n";
...
print OUT "Name\tstart\tend\tstrand\tchr";
