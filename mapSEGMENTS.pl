...
open (IN2, $in1) or die 'could not find the gff  file';
...
my @excluded=('CDS_motif','BLASTN_HIT','gap','repeat_unit','mRNA','rep_origin','polyA_signal','3\'UTR','5\'UTR','tRNA','promoter');
...
while ($line1=<IN1>)
{
 ...
 for ($i=$holder1[0];$i <= $holder1[1];$i++)
 {
  if (defined %{$HIT{$chr}{$i}})
  {
   if($HIT{$chr}{$i}{$holder1[4]} eq $prev)
   {
    next;
   }
   elsif ($HIT{$chr}{$i}{$holder1[4]}=~ /AS\.AS/)
   {
    #$counthits++;
    #$FINALHIT{$seg}.="\tFOUND".$HIT{$chr}{$i}{$holder1[4]};
    next;
   }
   else
   {
    $counthits++;
    $FINALHIT{$seg}.="\t$HIT{$chr}{$i}{$holder1[4]}";
   }
   $prev=$HIT{$chr}{$i}{$holder1[4]};
  }
  else
  {
   if ($prev eq "NEW")
 ...
 $out{$seg}="$seg\t$holder1[0]\t$holder1[1]\t$holder1[2]\t$holder1[3]\t$holder1[4]";
}
...
print OUT "Name\tstart\tend\tlength\tscore\tstrand\tchr";
